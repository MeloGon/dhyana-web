-- Agregar soporte de precio regular y descuento en talleres/horarios.
-- El precio final (price_cents) representa el valor efectivo a cobrar.
...
-- Para registros existentes, regular_price_cents es igual a su price_cents actual
update public.workshop_groups
set regular_price_cents = price_cents
where regular_price_cents is null;

-- Trigger para sincronizar y garantizar regular_price_cents ante inserciones o actualizaciones
create or replace function public.workshop_groups_sync_prices()
returns trigger
language plpgsql
as $$
begin
  if new.discount_cents is null then
    new.discount_cents := 0;
  end if;

  if TG_OP = 'INSERT' then
    if new.regular_price_cents is null then
      new.regular_price_cents := new.price_cents + new.discount_cents;
    end if;
    if new.price_cents is null then
      new.price_cents := new.regular_price_cents - new.discount_cents;
    end if;
  elsif TG_OP = 'UPDATE' then
    -- Si se actualizó price_cents directamente sin alterar regular_price_cents
    if new.price_cents is distinct from old.price_cents and new.regular_price_cents = old.regular_price_cents then
      new.regular_price_cents := new.price_cents + new.discount_cents;
    -- Si se actualizó regular_price_cents o discount_cents pero no price_cents
    elsif (new.regular_price_cents is distinct from old.regular_price_cents or new.discount_cents is distinct from old.discount_cents)
...
drop trigger if exists workshop_groups_sync_prices_trigger on public.workshop_groups;
create trigger workshop_groups_sync_prices_trigger
before insert or update on public.workshop_groups
for each row
execute function public.workshop_groups_sync_prices();

alter table public.workshop_groups
  alter column regular_price_cents set not null;

alter table public.workshop_groups
  add constraint workshop_groups_discount_check
  check (discount_cents < regular_price_cents and price_cents = regular_price_cents - discount_cents and price_cents > 0);

-- Actualizar save_workshop_catalog para persistir regular_price_cents y discount_cents
  ...
  for v_group in select value from jsonb_array_elements(p_workshop->'groups') loop
    ...
    if v_group_id is not null then
      ...
      insert into public.workshop_groups (
        workshop_id, schedule_description, price_cents, regular_price_cents, discount_cents,
