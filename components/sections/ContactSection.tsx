...
function Form({ service, services }: { service: string; services: string[] }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-[color:var(--ink)]/10 bg-[var(--surface)] shadow-sm">
...
export default function ContactSection({ content, contact, preselectedService }: Props) {
  ...
  return <section id="contacto" className="mx-auto max-w-7xl scroll-mt-20 px-4 py-20 sm:px-6 lg:px-8">
    {hasContact && <div className="mx-auto mb-12 max-w-3xl text-center">
      <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-[#6366F1]/15 text-[#6366F1] px-4 py-1 text-xs font-semibold uppercase tracking-wider"><Calendar className="h-4 w-4" /><span>{texts.contactEyebrow}</span></div>
