...
import { SaleActionForm } from '@/components/admin/sales/SaleActionForm';
  ...
  return <main className="mx-auto min-h-screen max-w-6xl px-5 py-10 sm:px-8">
    ...
    <form onSubmit={sales.search} className="mt-8 rounded-2xl bg-[#83D0C6]/10 p-5">
      <fieldset disabled={busy} className="grid items-end gap-4 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr_auto]">
        ...
        <div><label htmlFor="sales-access" className="text-sm font-medium">Acceso</label><select id="sales-access" value={sales.filters.access} onChange={(e) => sales.setFilters({ ...sales.filters, access: e.target.value as SalesFilters['access'] })} className={inputClass}>
          <option value="all">Todos</option><option value="active">Vigentes</option><option value="expired">Vencidos</option><option value="upcoming">Próximos</option><option value="cancelled">Anulados</option></select></div>
    ...
    <SalesList data={sales.data} isBusy={busy} coordinationId={sales.coordinationId} onCoordinate={sales.toggleCoordination} onPage={sales.goToPage} onAction={sales.openAction}
      actionForm={sales.actionTarget ? { saleId: sales.actionTarget.sale.id, content: <SaleActionForm
        ...
        onChange={sales.setActionInput} onSubmit={sales.submitAction} onClose={sales.closeAction}
        isPending={sales.actionPending} error={sales.actionError} /> } : null} />
  </main>;
}
