...
export function ManualSaleForm({ workshops, onSaved, onCancel }: {
  ...
  return (
    <form onSubmit={editor.handleSubmit} className="rounded-3xl border border-[color:var(--ink)]/10 bg-[var(--surface)] p-6 sm:p-8">
      ...
      {editor.error && <p role="alert" className="mt-4 text-sm text-[color:var(--danger)]">{editor.error}</p>}
