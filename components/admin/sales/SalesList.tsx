...
export function SalesList({ data, isBusy, coordinationId, onCoordinate, onPage, onAction, actionForm }: {
  ...
  return <section aria-label="Ventas registradas" className="mt-6">
    ...
    {!data.items.length && <p className="mt-5 rounded-2xl bg-[var(--surface)] p-7 text-sm">No hay ventas para estos filtros.</p>}
    <ul className="mt-4 space-y-4">
      {data.items.map((sale) => <li key={sale.id} className="rounded-2xl border border-[color:var(--ink)]/10 bg-[var(--surface)] p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <span className={`rounded-full px-3 py-1 text-xs font-medium ${sale.accessStatus === 'active' ? 'bg-[#83D0C6]/25 text-[#285D48] dark:text-[#A8E5CD]' : 'bg-[#F0E8DF] text-[#6B5140]'}`}>{ACCESS_LABELS[sale.accessStatus]}</span>
        </div>
        <p className="mt-4 font-medium">{sale.workshopTitle}</p><p className="mt-1 text-sm">{sale.scheduleDescription}</p>
        <dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2">
          <div><dt className="text-[color:var(--ink)]/70">Pago registrado</dt><dd className="mt-1">{formatPrice(sale.amountCents)} · {sale.origin === 'culqi' ? 'Culqi' : MANUAL_PAYMENT_METHODS.find((method) => method.value === sale.paymentMethod)?.label ?? 'Venta manual'}</dd><dd className="mt-1">{formatLimaDate(sale.purchasedAt)}</dd></div>
          <div><dt className="text-[color:var(--ink)]/70">{sale.status === 'cancelled' ? 'Período original (hora de Perú)' : 'Acceso mensual (hora de Perú)'}</dt><dd className="mt-1">Desde {formatLimaDate(sale.startsAt)}</dd><dd className="mt-1">Hasta {formatLimaDate(sale.endsAt)}</dd></div>
          <div className="min-w-0"><dt className="text-[color:var(--ink)]/70">Código de compra</dt><dd className="mt-1 break-all font-mono text-xs">{sale.referenceCode}</dd></div>
          {sale.paymentReference && <div className="min-w-0"><dt className="text-[color:var(--ink)]/70">Referencia del pago</dt><dd className="mt-1 break-all">{sale.paymentReference}</dd></div>}
        </dl>
        ...
        {sale.cancelledAt && <p className="mt-4 text-sm text-[color:var(--danger)]">Anulada el {formatLimaDate(sale.cancelledAt)} · {sale.cancellationReason}</p>}
        <div className="mt-4 flex flex-wrap gap-4 text-sm text-[color:var(--danger)]">
