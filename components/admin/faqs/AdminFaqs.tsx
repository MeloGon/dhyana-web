    ...
    <main className="mx-auto min-h-screen max-w-6xl px-5 py-10 sm:px-8">
      <header className="flex flex-wrap items-start justify-between gap-5 border-b border-[color:var(--ink)]/15 pb-7">
        <div>
          ...
          <p className="mt-3 max-w-xl text-sm text-[color:var(--ink)]/75">Edita las preguntas de la sección de contacto. Los cambios publicados se ven al volver a cargar el sitio.</p>
        </div>
        <button onClick={() => model.editFaq()} disabled={model.isSaving} className="rounded-xl bg-[var(--mint-solid)] px-5 py-3 font-medium disabled:opacity-60">Nueva pregunta</button>
      </header>
      ...
      {model.pendingDelete && (
        <section role="region" aria-label="Confirmar eliminación" className="mt-6 rounded-2xl border border-[#9B3024]/30 bg-[var(--surface)] p-5">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <button onClick={() => model.setPendingDelete(null)} disabled={model.isSaving} className="rounded-xl border border-[color:var(--ink)]/25 px-4 py-2 disabled:opacity-60">Cancelar</button>
          </div>
        </section>
      )}
      <div className="mt-8 grid items-start gap-7 lg:grid-cols-2">
        <section aria-label="Preguntas guardadas" className="space-y-4">
          {model.faqs.length === 0 && <p className="rounded-2xl bg-[var(--surface)] p-6 text-sm">Todavía no hay preguntas. Crea la primera con el formulario.</p>}
          {model.faqs.map((faq) => (
            <article key={faq.id} className={`rounded-2xl border bg-[var(--surface)] p-5 ${model.selectedId === faq.id ? 'border-[#467E76]' : 'border-[color:var(--ink)]/10'}`}>
              <p className="text-xs font-medium text-[color:var(--positive)]">Orden {faq.sortOrder} · {faq.isPublished ? 'Publicada' : 'Borrador'}</p>
              <h2 className="mt-2 break-words font-semibold">{faq.question}</h2>
              <p className="mt-2 line-clamp-3 whitespace-pre-line break-words text-sm leading-relaxed text-[color:var(--ink)]/75">{faq.answer}</p>
              <div className="mt-4 flex gap-5 text-sm">
                ...
                <button onClick={() => model.setPendingDelete(faq)} disabled={model.isSaving} className="text-[color:var(--danger)] underline underline-offset-4 disabled:opacity-60">Eliminar</button>
              </div>
            </article>
          ))}
        </section>
