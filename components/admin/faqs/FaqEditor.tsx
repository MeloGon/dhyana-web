...
export function FaqEditor({ form, isEditing, isSaving, isDisabled, onChange, onSubmit }: Props) {
  return (
    <form onSubmit={(event) => { event.preventDefault(); onSubmit(); }} className="rounded-3xl border border-[color:var(--ink)]/10 bg-[var(--surface)] p-6 sm:p-8">
      ...
      <fieldset disabled={isSaving || isDisabled} className="mt-6 space-y-5 disabled:opacity-60">
        ...
        <label className="block text-sm font-medium">Orden de aparición
          ...
          <span id="faq-order-help" className="mt-2 block text-xs font-normal text-[color:var(--ink)]/75">Los números menores aparecen primero. Usa números distintos para elegir el orden.</span>
        </label>
        ...
        <p className="text-xs text-[color:var(--ink)]/75">Si desmarcas esta opción, la pregunta queda guardada como borrador.</p>
