  ...
  return (
    <main className="mx-auto min-h-screen max-w-6xl px-5 py-10 sm:px-8">
      ...
      {model.successMessage && (
        <p role="status" className="mt-5 text-sm text-[color:var(--positive)]">
          {model.successMessage}
        </p>
      )}
        ...
        >
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <button
              ...
              disabled={model.isSaving}
              className="rounded-xl border border-[color:var(--ink)]/25 px-4 py-2 disabled:opacity-60 cursor-pointer"
            >
              Cancelar
            </button>
          </div>
      ...
      <div className="mt-8 grid items-start gap-7 lg:grid-cols-2">
        <section aria-label="Opiniones guardadas" className="space-y-4">
          ...
          {model.quotes.map((q) => (
            <article
              key={q.id}
              className={`rounded-2xl border bg-[var(--surface)] p-5 transition-all ${
                model.selectedId === q.id ? 'border-[#467E76] shadow-sm' : 'border-[color:var(--ink)]/10'
              }`}
            >
              <div className="flex items-center justify-between gap-2">
                <span className="text-xs font-medium text-[color:var(--positive)]">
                  Orden {q.sortOrder} · {q.isPublished ? 'Publicada' : 'Borrador'}
                </span>
                <span className="rounded-full bg-[#3D4C5A]/5 px-2.5 py-0.5 text-xs text-[color:var(--ink)]/70">
                  {variantLabels[q.variant] ?? q.variant}
                </span>
              </div>

              <blockquote className="mt-2.5 font-serif italic text-base text-[color:var(--ink)] leading-snug">
                &ldquo;{q.quote}&rdquo;
              </blockquote>

              <p className="mt-2 text-sm font-semibold text-[color:var(--ink)]">{q.author}</p>
              {q.role && <p className="text-xs text-[color:var(--ink)]/70">{q.role}</p>}
              {q.accentNote && (
                <p className="mt-2 font-handwriting text-lg text-[color:var(--positive)]">
                  ~ {q.accentNote} ~
                </p>
              )}

              <div className="mt-4 flex gap-5 text-sm border-t border-[color:var(--ink)]/10 pt-3">
                <button
                ...
                <button
                  ...
                  disabled={model.isSaving}
                  className="text-[color:var(--danger)] underline underline-offset-4 disabled:opacity-60 cursor-pointer"
                >
                  Eliminar
                </button>
              </div>
