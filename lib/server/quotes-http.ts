...
import { deleteQuote, getAdminQuotes, getPublicQuotes, saveQuote } from '@/lib/server/quotes';
...
function errorResponse(error: unknown) {
  return error instanceof HttpError ? json({ message: error.message }, error.status)
    : json({ message: 'No se pudo acceder a las citas y reflexiones. Inténtalo nuevamente.' }, 503);
}
...
export async function handleQuotesGet(isAdmin: boolean) {
  try { return json(isAdmin ? await getAdminQuotes() : await getPublicQuotes()); }
  catch (error) { return errorResponse(error); }
}

export async function handleQuoteSave(request: Request, id?: string) {
  try { return json(await saveQuote(await readBody(request), id), id ? 200 : 201); }
  catch (error) { return errorResponse(error); }
}

export async function handleQuoteDelete(request: Request, id: string) {
  try { await readBody(request); return json(await deleteQuote(id)); }
  catch (error) { return errorResponse(error); }
}
