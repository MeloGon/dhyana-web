...
import { getAdminLegalSettings, getPublicLegalSettings, saveLegalSettings } from '@/lib/server/legal-settings';
...
function errorResponse(error: unknown) {
  return error instanceof HttpError ? json({ message: error.message }, error.status)
    : json({ message: 'No se pudo acceder a los términos y políticas. Inténtalo nuevamente.' }, 503);
}

export async function handleLegalSettingsGet(isAdmin: boolean) {
  try { return json(isAdmin ? await getAdminLegalSettings() : await getPublicLegalSettings()); }
  catch (error) { return errorResponse(error); }
}

export async function handleLegalSettingsSave(request: Request) {
  try {
    ...
    if (text.length > 200000) throw new HttpError(413, 'Solicitud demasiado grande.');
    let body: unknown;
    ...
    return json(await saveLegalSettings(body as Record<string, unknown>));
  } catch (error) { return errorResponse(error); }
}
