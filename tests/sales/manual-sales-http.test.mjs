...
test('ventas: visitante sin acceso a participantes, registro ni coordinación', async () => {
  for (const [path, method] of [['/api/admin/sales', 'GET'], ['/api/admin/sales', 'POST'],
    [`/api/admin/sales/${randomUUID()}/coordination`, 'PATCH']]) {
...
test('ventas: mutaciones rechazan origen externo antes de procesar datos', async () => {
  for (const [path, method] of [['/api/admin/sales', 'POST'], [`/api/admin/sales/${randomUUID()}/coordination`, 'PATCH']]) {
