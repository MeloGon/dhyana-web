...
/** Hook para la landing pública: carga las opiniones con AbortController y soporte de reintento. */
export function usePublicQuotes() {
  ...
  useEffect(() => {
    ...
    getPublicQuotes(controller.signal)
      ...
      .catch(() => {
        if (!controller.signal.aborted) setErrorMessage('No pudimos cargar las opiniones.');
      })
      .finally(() => {
...
export function useAdminQuotes(initial: AdminQuote[]) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setForm(saved);
      setSuccessMessage(
        saved.isPublished ? 'Opinión guardada y publicada.' : 'Opinión guardada como borrador.'
      );
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'No se pudo guardar la opinión.');
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      setPendingDelete(null);
      setSuccessMessage('Opinión eliminada.');
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : 'No se pudo eliminar la opinión.');
    } finally {
      setIsSaving(false);
    }
  }
